...
# Образы и их версии (должны совпадать с теми, что в deployment.yaml)
...
# Пути к манифестам
MANIFESTS_GLOBAL := the_project/manifests
MANIFESTS_LOG := log_output/manifests
MANIFESTS_PINGPONG := the_project/pingpong/manifests
...
build:
	...
	docker build -t $(IMG_WRITER) ./log_output/writer
	docker build -t $(IMG_READER) ./log_output/reader
	docker build -t $(IMG_PINGPONG) ./the_project/pingpong
...
# Порядок важен: сначала глобальные ресурсы (PV/PVC), потом приложения
deploy:
...
# Порядок обратный: сначала удаляем приложения, потом глобальные ресурсы
delete:
	...
	kubectl delete -f $(MANIFESTS_GLOBAL)/ --ignore-not-found=true
	@echo "✅ Delete complete."

# 5. Перезапуск всех Deployment'ов (Самая частая команда при изменении кода!)
restart:
	@echo "🔄 Restarting deployments to pick up new images..."
	kubectl rollout restart deployment log-output-deployment
	kubectl rollout restart deployment pingpong-deployment
	kubectl rollout restart deployment todo-app-deployment
	@echo "✅ Restart initiated. Watch pods with: make logs"

# 6. Очистка локальных образов (освобождает место на диске)
clean:
	@echo "🧹 Cleaning up local Docker images..."
	docker rmi $(IMG_WRITER) $(IMG_READER) $(IMG_PINGPONG) $(IMG_TODO) --force || true
	@echo "✅ Clean complete."

# 7. Удобный просмотр логов всех подов в реальном времени
logs:
...
help:
	...
	@echo "  make deploy   - Apply all Kubernetes manifests (kubectl apply)"
	@echo "  make delete   - Delete all Kubernetes manifests (kubectl delete)"
	@echo "  make restart  - Restart all deployments (use after code changes)"
